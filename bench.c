...
#define BENCH_MAT_CREATE_COUNT      100000
#define pbench(...)      phylw("\n[Benchmark] ", __VA_ARGS__); fflush(stdout);

static clock_t start, end;
static Matrix matrices[BENCH_MAT_CREATE_COUNT], result[BENCH_MAT_CREATE_COUNT/2];
...
static void free_result(){
    for(int i = 0;i < BENCH_MAT_CREATE_COUNT/2; i++)
        mat_free(result[i]);
}

void bench_matrix_mult(){
